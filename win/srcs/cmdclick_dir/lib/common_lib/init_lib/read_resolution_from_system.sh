...
read_resolution_from_system(){
	...
	CENTER_SCALE_DISPLAY_HEIGHT="$(\
		echo "scale=0; ${DISPLAY_RSOLUTION_HEIGHT} / 1.1" \
		| bc\
	)"
	CENTER_SCALE_DISPLAY_WIDTH="$(\
		echo "scale=0; ${DISPLAY_RSOLUTION_WIDTH} / 1.9" \
		| bc\
	)"
	...
	CENTER_X_POSITION="$(\
		echo "scale=0; (${true_display_resolution_width} - ${CENTER_SCALE_DISPLAY_WIDTH}) / 2" \
		| bc\
	)"
	CENTER_Y_POSITION="$(\
		echo "scale=0; (${DISPLAY_RSOLUTION_HEIGHT} - ${CENTER_SCALE_DISPLAY_HEIGHT}) / 2" \
		| bc\
	)"
	RIGHT_SCALE_DISPLAY_WIDTH=$(\
		echo "scale=0; ${true_display_resolution_width} / 3" \
		| bc\
	)
	RIGHT_SCALE_DISPLAY_HEIGHT=$(\
		echo "scale=0; ${DISPLAY_RSOLUTION_HEIGHT} / 1.2  - ${PANAL_HEIGHT}" \
		| bc\
	)
	RIGHT_X_POSITION=$(\
		echo "scale=0; \
	...
	RIGHT_Y_POSITION=$(\
		echo "scale=0; ${DISPLAY_RSOLUTION_HEIGHT} - ${RIGHT_SCALE_DISPLAY_HEIGHT} - ${PANAL_HEIGHT}" \
		| bc\
	)	
}