...
readonly INI_SETTING_KEY_LIST=("${INI_TERMINAL_ON}" "${INI_OPEN_WHERE}" "${INI_TERMINAL_FOCUS}" "${INI_INPUT_EXECUTE}" "${INI_IN_EXE_DFLT_VL}" "${INI_INPUT_EXEC_ROOP_DO}" "${INI_BEFORE_COMMAND}" "${INI_AFTER_COMMAND}" "${INI_CMD_FILE_NAME}")
...
readonly INI_INPUT_EXEC_ROOP_DO_VALUE_CATEGORY=":CB"
...
${INI_INPUT_EXEC_ROOP_DO_VALUE_CATEGORY}
