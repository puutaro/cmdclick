...
readonly INI_SETTING_KEY_LIST=("${INI_TERMINAL_ON}" "${INI_OPEN_WHERE}" "${INI_TERMINAL_FOCUS}" "${INI_EDIT_EXECUTE}" "${INI_IN_EXE_DFLT_VL}" "${INI_INPUT_EXEC_ROOP_DO}" "${INI_INPUT_EXEC_DFLT_SAVE_DO}" "${INI_BEFORE_COMMAND}" "${INI_AFTER_COMMAND}" "${INI_CMD_FILE_NAME}")
