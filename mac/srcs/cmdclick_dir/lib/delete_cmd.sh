...
delete_cmd(){
  ...
  local delete_contents="$(echo "${delete_contents}" | gsed -e "s/${CMDCLICK_N_CAHR}/n/g" -e 's/'${CMDCLICK_BACKSLASH_MARK}'/\\/g' -e 's/'${CMDCLICK_ONE_BACKSLASH_MARK}'/\\/g')"
  ...
  if [ -n "${VALUE}" ];then 
    if [ "${CHDIR_SIGNAL_CODE}" == "${DELETE_CODE}" ];then
        ...
        if [ -n "${CD_DEL_VALUE}" ];then 
          ...
          gsed -i 's/^'${GREP_SECONDS_INI_FILE_DIR_PATH}'\=.*/'${GREP_SECONDS_INI_FILE_DIR_PATH}'\='${sed_ch_dir_path}'/' "${CMDCLICK_PROPERTY_FILE_PATH}"
        fi
    fi
