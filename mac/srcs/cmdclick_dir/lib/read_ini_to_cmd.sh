...
read_ini_to_execute_command(){
	...
	local source_con=$(cat "${1}" | sed -n '/'${SEARCH_INI_SETTING_SECTION_START_NAME}'/,/'${SEARCH_INI_SETTING_SECTION_END_NAME}'/p' | sed '/'${SEARCH_INI_SETTING_SECTION_START_NAME}'/d' | sed '/'${SEARCH_INI_SETTING_SECTION_END_NAME}'/d' | rga "^[a-zA-Z0-9_-]{1,100}=")
