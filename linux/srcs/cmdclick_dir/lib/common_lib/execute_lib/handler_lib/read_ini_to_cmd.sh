...
read_ini_to_execute_command(){
	...
	EXEC_INPUT_EXEC_DFLT_SAVE_DO="${variabl_contensts_setting_value_list[6]}"
	EXEC_BEFORE_COMMAND="$(echo "${variabl_contensts_setting_value_list[7]}" | sed -e 's/^"//' -e 's/"$//')"
	EXEC_AFTER_COMMAND="$(echo "${variabl_contensts_setting_value_list[8]}" | sed -e 's/^"//' -e 's/"$//')"
}
